...
class TunerAPI:
    ...
    def get_orders(self, id):
        query = f"SELECT channelUuid, start, stop, channelName, title, subtitle, summary, description, favorite, alreadySaved FROM record_orders \
            WHERE tuner_id = {id}"
        ...
        result = [
            {"channelUuid": str(o[0]), "start": o[1], "stop": o[2], "channelName": str(o[3]), "title": str(o[4]), "subtitle": str(o[5]), "summary": str(o[6]), "description": str(o[7]), "favorite": bool(o[8]), "alreadySaved": bool(o[9])}
            for o in orders
            if o[1] > ts
        ]
